...
def seed(image,mask,increment,stopval): #Find Seeds to grow where Seed is found as isolated maximum within max and increment but >stopval
	...
	for y in range(mask.shape[0]):	#fill temporary mask with 0:no region and 1:any region
					...
					while condition:
						for cent in chck:
							if flr_mask[cent[0]+1,cent[1]]==-1:
								...
								flr_mask[cent[0]+1,cent[1]]=reg_no
							...
							if flr_mask[cent[0],cent[1]+1]==-1:
								...
								flr_mask[cent[0],cent[1]+1]=reg_no
...
while loop_cond:
	print loop_count
	new_mask,maximumval = seed(dataset,run_mask,0.2,low_level)
	if new_mask == run_mask:
		final_mask = new_mask * 1
		loop_cond = False	

	for i in range(grow_loops):
		new_mask = grow(dataset,new_mask,maximumval,0.015,low_level)
