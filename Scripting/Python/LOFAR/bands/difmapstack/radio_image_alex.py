...
input_path_image = 'modeltest1.fits'
...
scale_min = -0.0053556 #if 'None': defined automatically
...
loop_path = 'modeltest1.fits'
hdul = fits.open(loop_path)
neu = hdul[0].data
if len(neu)==1:
	image = neu[0][0]
else:
...
for i in range(2,12):
	loop_path = 'modeltest'+str(i)+'.fits'
	hdul = fits.open(loop_path)
	neu = hdul[0].data
	if len(neu)==1:
		image_loop = neu[0][0]
		...
		image_loop = neu
	image = image + np.array(image_loop)
